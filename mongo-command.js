...
db.books.insertMany([
  ...
  db.book.find({rating: { "$gte": 8 }}).pretty();

  db.book.find({rating: { "$lte": 7 }}).pretty();

  db.book.find({}, { name: 1, rating:1}).pretty();

  db.book.find({}, { name: 0, rating:0}).pretty();

  db.book.find({}, { summary: 0, trailer:0}).pretty();

  db.book.find({}).sort ({rating:-1}).pretty();

  db.book.find({}).sort ({rating:1}).pretty();

  db.book.find({}).sort ({rating:-1}).limit(3).pretty();

  db.book.find({}).sort ({rating:-1}).skip(2).pretty();

  db.book.find({rating:{$gt:8}}, {_id:0, name:1, rating:1}).sort({rating: -1}).pretty();
